...
function networkDown() {
  # stop AAD containers also in addition to org1 and org2, in case we were running sample to AAD AAD
  docker-compose -f $COMPOSE_FILE_BASE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_CA down --volumes --remove-orphans
  docker-compose -f $COMPOSE_FILE_COUCH_AAD -f $COMPOSE_FILE_AAD down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf addAAD/fabric-ca/AAD/msp addAAD/fabric-ca/AAD/tls-cert.pem addAAD/fabric-ca/AAD/ca-cert.pem addAAD/fabric-ca/AAD/IssuerPublicKey addAAD/fabric-ca/AAD/IssuerRevocationPublicKey addAAD/fabric-ca/AAD/fabric-ca-server.db'
...
# use this as the docker compose couch file for AAD
COMPOSE_FILE_COUCH_AAD=AAD/docker/docker-compose-couch-AAD.yaml
# use this as the default docker-compose yaml definition for AAD
COMPOSE_FILE_AAD=AAD/docker/docker-compose-AAD.yaml
#
